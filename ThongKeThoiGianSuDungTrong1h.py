...
user = '092e8def-ca77-45d3-b834-c5bde45a7f1d'

fig, axes = plt.subplots(4, 7, figsize=(21, 9))
i=0
j=0

user_path = f'data/{user}'  

for device_type in os.listdir(user_path):
    device_type_path = os.path.join(user_path, device_type)
    for device_id in os.listdir(device_type_path):
        device_id_path = os.path.join(device_type_path, device_id)
        i=0
        j=0
        for filename in os.listdir(device_id_path):
            if filename.endswith('.csv'): 
                file_path = os.path.join(device_id_path, filename)
                ...
                ax = axes[i, j]
                ax.plot(hours, values, label = device_id)
